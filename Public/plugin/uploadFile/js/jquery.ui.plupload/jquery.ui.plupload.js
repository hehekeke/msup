...
$.widget("ui.plupload", {
	...
	_initUploader: function() {
		...
		uploader.bind('FileUploaded', function(up, file,rep) {
			if ( rep.fileName ) {
				file.name = rep.fileName;
			}

			self._handleFileStatus(file);
			self._trigger('uploaded', null, { up: up, file: file } );
			response = JSON.parse(rep.response);
			self._changeFileName(file.id, response.fileName);
			self._addFormFields(response, file.id);
		});
	...
	_changeFileName: function(id, newName) {

		var ele = $("#"+id).find(".plupload_file_name");
		ele.attr("title", newName);
		ele.find("span").text(newName);
	...
	*/
	...
	//ZYH 添加表单字段
	_addFormFields: function(response, id) {
		var self = this;
		var name = $("#"+self.id).attr("name");
		var  ele  =  $("#"+id); 
		// re-add from fresh

			if (typeof response !== 'undefined') {
				var value = '{"fileName":"'+response.fileName+'","fileUrl":"'+response.fileUrl+'"}';
				var fields = ''
				, id = name;
				;

				fields 	= '<input type="hidden" class="puploadFiles" name=\'' +name + '\' value=\''+value+'\' />';
				ele.find('.plupload_file_fields').append(fields);
			}
		...
		this.filelist.dblclick(function(e) {
			...
			nameInput.val(name).blur(function() {
				...
				if ($.inArray(e.keyCode, [13, 27]) !== -1) {
					...
					nameInput.parent().parent().find(".plupload_file_fields .fileName").val(file.name);
					nameInput.blur();
				}
			})[0].focus();
		});
...
function getFieldValue(ele, fieldValue) {
	...
	if (ele.find("input.puploadFiles").length < 0) return fieldValue;

	fieldValue = (fieldValue == '') ? '[' : fieldValue;

	ele.find("input.puploadFiles").each(function () {
